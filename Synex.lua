...
function Linux.Create(config)
    ...
    -- This function will be used for searching elements
    local function searchElements(searchText)
        -- First search through tabs
        ...
        -- Then search through elements in the current tab
        if CurrentTab and Tabs[CurrentTab] then
            local container = Tabs[CurrentTab].Content:FindFirstChildOfClass("ScrollingFrame")
            if container then
                for _, element in pairs(container:GetChildren()) do
                    if element:IsA("Frame") then
                        local elementText = element:FindFirstChildOfClass("TextLabel") or element:FindFirstChildOfClass("TextButton")
                        if elementText then
                            if searchText == "" then
                                element.Visible = true
                            else
                                local elementName = string.lower(elementText.Text)
                                element.Visible = string.find(elementName, searchText) ~= nil
                            end
                        end
                    end
                end
    ...
    -- Connect the search function to the search box
    ...
    function LinuxLib.Tab(config)
        ...
        local function SelectTab()
            ...
            if SearchBox.Text ~= "" then
                searchElements(string.lower(SearchBox.Text))
            end
        end
        ...
        function TabElements.Dropdown(config)
            ...
            -- Change from purple to white for dropdown selected text
                ...
                if IsOpen then
                    for i, opt in pairs(Options) do
                        local OptBtn = Linux.Instance("TextButton", {
                            ...
                            Font = Enum.Font.GothamSemibold,
                            Text = tostring(opt),
                            TextColor3 = opt == SelectedValue and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(200, 200, 210),
                            TextSize = 14,
                            ZIndex = 3,
                            AutoButtonColor = false,
                            LayoutOrder = i
