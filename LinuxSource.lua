...
function Linux.Create(config)
    ...
    Linux.Instance("UICorner", {
        Parent = TopBar,
        CornerRadius = UDim.new(0 well, 8)
    })
    ...
    SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        for _, tab in pairs(TabHolder:GetChildren()) do
            if tab:IsA("TextButton") then
                local tabText = tab:FindFirstChildOfClass("TextLabel")
                if tabText then
                    if searchText == "" then
                        tab.Visible = true
                    else
                        local tabName = string.lower(tabText.Text)
    ...
    function LinuxLib.Tab(config)
        ...
        function TabElements.Button(config)
            ...
            local accentColor = Color3.fromRGB(80, 80, 80)
            local hoverColor = Color3.fromRGB(50, 50, 50)
            local originalColor = Color3.fromRGB(25, 25, 30)
            local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            
            Btn.MouseEnter:Connect(function()
                TweenService:Create(BtnFrame, tweenInfo, {BackgroundColor3 = hoverColor, BackgroundTransparency = 0.5}):Play()
            ...
            Btn.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
                    TweenService:Create(BtnFrame, tweenInfo, {BackgroundColor3 = accentColor, BackgroundTransparency = 0.5}):Play()
                end
            end)
        ...
        function TabElements.Toggle(config)
            ...
            local accentColor = Color3.fromRGB(180, 180, 180)
            local darkAccentColor = Color3.fromRGB(140, 140, 140)
            ...
            local function UpdateToggle(thisToggle)
                ...
                if thisTrack and thisKnob then
                    ...
                    if currentState then
                        TweenService:Create(thisTrack, tween, {BackgroundColor3 = accentColor, BackgroundTransparency = 0.4}):Play()
                        thisTrack:FindFirstChildOfClass("UIGradient").Enabled = true
                        TweenService:Create(thisKnob, tween, {Position = UDim2.new(0, 20, 0.5, -7), BackgroundColor3 = Color3.fromRGB(255, 255, 255)}):Play()
                    else
                        TweenService:Create(thisTrack, tween, {BackgroundColor3 = Color3.fromRGB(40, 40, 50), BackgroundTransparency = 0}):Play()
                        thisTrack:FindFirstChildOfClass("UIGradient").Enabled = false
        ...
        function TabElements.Dropdown(config)
            ...
            local DropdownButton = Linux.Instance("TextButton", {
                Parent = Dropdown,
                BackgroundTransparency = 1,
                Size = UDim2.new( Reissuing 1, 0, 1, 0),
                Font = Enum.Font.GothamSemibold,
                Text = "",
                TextColor3 = Color3.fromRGB(200, 200, 210),
                TextSize = 14,
            ...
            local accentColor = Color3.fromRGB(130, 120, 180)
            local darkAccentColor = Color3.fromRGB(100, 90, 150)
                    ...
                    for i, opt in pairs(Options) do
                        ...
                        OptBtn.MouseButton1Click:Connect(function()
                            if opt ~= SelectedValue then
                                ...
                                Selected.TextColor3 = accentColor
        ...
        function TabElements.Colorpicker(config)
            ...
            local ColorCanvas = Linux.Instance("ImageLabel", {
                Parent = PickerFrame,
                BackgroundTransparency = 1,
            ...
            local CanvasSelector = Linux.Instance("Frame", {
                Parent = ColorCanvas,
                BackgroundColor3 = Color3.fromRGB(255, 255, 255),
            ...
            local HueSelector = Linux.Instance("Frame", {
                Parent = HueBar,
                BackgroundColor3 = Color3.fromRGB(255, 255, 255),
            ...
            local canvasPos = Vector2.new(s * ColorCanvas.AbsoluteSize.X, (1 - v) * ColorCanvas.AbsoluteSize.Y)
            local huePos = h * HueBar.AbsoluteSize.X
            
            CanvasSelector.Position = UDim2.new(0, canvasPos.X - 3, 0, canvasPos.Y - 3)
            HueSelector.Position = UDim2.new(0, huePos - 2, 0, -2)
            
            local function UpdateColor()
                local canvasSize = ColorCanvas.AbsoluteSize
                local canvasPos = CanvasSelector.Position
                local hueSize = HueBar.AbsoluteSize
                local huePos = HueSelector.Position
                
                local s = math.clamp((canvasPos.X.Offset + 3) / canvasSize.X, 0, 1)
                local v = math.clamp(1 - (canvasPos.Y.Offset + 3) / canvasSize.Y, 0, 1)
                local h = math.clamp((huePos.X.Offset + 2) / hueSize.X, 0, 1)
            ...
            local function TogglePicker()
                ...
                if isPicking then
                    ...
                    TweenService:Create(PickerFrame, tween, {Size = UDim2.new(1, 0, 0, 140)}):Play()
                else
                    TweenService:Create(PickerFrame, tween, {Size = UDim2.new(1, 0, 0, 0)}):Play()
                    task.delay(0.25, function()
            ...
            ColorCanvas.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    local relativeX = math.clamp(pos.X - canvasPos.X, 0, canvasSize.X)
                    local relativeY = math.clamp(pos.Y - canvasPos.Y, 0, canvasSize.Y)
                    CanvasSelector.Position = UDim2.new(0, relativeX - 3, 0, relativeY - 3)
                    UpdateColor()
                end
            end)
            
            ColorCanvas.InputChanged:Connect(function(input)
                if (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement) and draggingCanvas then
                    local pos = input.Position
                    local canvasSize = ColorCanvas.AbsoluteSize
                    local canvasPos = ColorCanvas.AbsolutePosition
                    local relativeX = math.clamp(pos.X - canvasPos.X, 0, canvasSize.X)
            ...
            HueBar.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    local relativeX = math.clamp(pos.X - huePos.X, 0, hueSize.X)
                    HueSelector.Position = UDim2.new(0, relativeX - 2, 0, -2)
                    UpdateColor()
                end
            end)
            ...
            local function SetValue(newColor)
                if typeof(newColor) == "Color3" then
                    ...
                    ColorPreview.BackgroundColor3 = currentColor
                    local h, s, v = currentColor:ToHSV()
                    CanvasSelector.Position = UDim2.new(0, s * ColorCanvas.AbsoluteSize.X - 3, 0, (1 - v) * ColorCanvas.AbsoluteSize.Y - 3)
                    HueSelector.Position = UDim2.new(0, h * HueBar.AbsoluteSize.X - 2, 0, -2)
                    spawn(function() Linux:SafeCallback(config.Callback, currentColor) end)
                end
            end
